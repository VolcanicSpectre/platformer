...
class Genome:
                ...
                for hidden_node in [node_gene for node_gene in self.node_genes if node_gene.type_ == NodeTypes.HIDDEN]:
                    if self.rng.uniform(size=1) < PERCENTAGE_OF_INITIAL_CONNECTIONS:
                        ...
                        self.connection_genes.append(ConnectionGene(innovation_id, input_node.id, hidden_node.id,
                                                                    self.rng.uniform(low=-20, high=20, size=1)[0]))
    ...
    def feed_forward(self, inputs):
        ...
        for input_node, input_value in zip(node_genes_by_layer[0], inputs):
            ...
            for connection_gene_terminating_at_node in [connection_gene for connection_gene in self.connection_genes if
                                                        connection_gene.enabled and connection_gene.output_node_id == input_node.id]:
                input_node.input_value += self.node_genes[
                                              connection_gene_terminating_at_node.input_node_id].output_value * connection_gene_terminating_at_node.weight
        ...
        for layer in node_genes_by_layer.values():
            for node_gene in layer:
                ...
                for connection_gene_terminating_at_node in [connection_gene for connection_gene in self.connection_genes
                                                            if connection_gene.enabled and
                                                               connection_gene.output_node_id == node_gene.id]:
                    node_gene.input_value += self.node_genes[
                                                 connection_gene_terminating_at_node.input_node_id].output_value * connection_gene_terminating_at_node.weight
        ...
        for i in range(20):
            ...
            if connection_type:
                ...
                if connection_type == ConnectionTypes.CONNECTION_EXISTS:
                    ...
                    if self.rng.uniform(size=1) < RECURRENT_CONNECTION_CHANCE and ALLOW_RECURRENT_CONNECTIONS:
                        recurrent = True
                    ...
                    self.connection_genes.append(ConnectionGene(innovation_id, node_gene_1.id, node_gene_2.id,
                                                                self.rng.uniform(low=-20, high=20, size=1)[0],
                                                                recurrent=connection_type == recurrent))
    ...
    def mutate_add_node(self):
        connection_for_new_node = self.rng.choice(
            [connection_gene for connection_gene in self.connection_genes if not connection_gene.recurrent])
        ...
        new_connection_gene_1 = ConnectionGene(innovation_id, connection_for_new_node.input_node_id, new_node_gene.id,
                                               self.rng.uniform(low=-20, high=20, size=1)[0])
        ...
        new_connection_gene_2 = ConnectionGene(innovation_id, new_node_gene.id, connection_for_new_node.output_node_id,
                                               self.rng.uniform(low=-20, high=20, size=1)[0])
    ...
    def get_longest_path_to_input_layer(self, node_gene):
        ...
        for connection_termianting_at_node in [connection_gene for connection_gene in self.connection_genes if (
                                                                                                                       not connection_gene.recurrent) and connection_gene.output_node_id == node_gene.id]:
    ...
    def draw(self, screen):
        ...
        row_heights_dict = {layer_number: (1080 // nodes_in_layer) for layer_number, nodes_in_layer in
                            zip(rows.keys(), [len(node_genes) for node_genes in rows.values()])}
        column_pixels = list(range(0, 1921, column_width))
        row_pixels = {layer_number: list(range(0, 1081, row_height)) for layer_number, row_height in
                      zip(row_heights_dict.keys(), row_heights_dict.values())}
        ...
        for node_id, params in zip(nodes_to_draw.keys(), nodes_to_draw.values()):
            ...
            for connection_gene in [connection_gene for connection_gene in self.connection_genes if
                                    node_id == connection_gene.input_node_id]:

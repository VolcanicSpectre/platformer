...
class Tileset:
    ...
    def get_tile_from_tileset(self, identifier: int) -> Surface:
        ...
        """
        grid_tile_x = identifier - (
            self.tile_grid_size * (identifier // self.tile_grid_size)
        )
        pixel_tile_x = self.padding + (
            grid_tile_x * (self.spacing + self.tile_grid_size)
        )

        grid_tile_y = identifier // self.tile_grid_size
        pixel_tile_y = self.padding + (
            grid_tile_y * (self.spacing + self.tile_grid_size)
        )

        handle_tileset = self.tileset_image.copy()
        clip_rect = Rect(
            pixel_tile_x, pixel_tile_y, self.tile_grid_size, self.tile_grid_size
        )

...
class TileSet:
    def __init__(self, tileset_path):
        with open(tileset_path) as f:
            data = json.load(f)
            self.image = pygame.image.load(path.abspath(
                path.join(tileset_path, "..", data["image"])))
            self.margin = data["margin"]
            self.spacing = data["spacing"]
            self.tile_width = data["tilewidth"]
            self.tile_height = data["tileheight"]
            self.tileset_width = math.floor(
                (data["imagewidth"] - self.margin + self.spacing) / (self.tile_width + self.spacing))
...
def generate_map_data(level_path, chunk_size):
    ...
    tile_set = TileSet(path.abspath(
        path.join(level_path, "..", map_data["tilesets"][0]["source"])))

...
class IdleState(State):
    def handle_inputs(self):
        if self.entity.direction: return States.RUN
        if self.entity.events["up"]: return States.JUMP
        
        return States.NULL
    
    def process_y_movement(self, dt):
        self.entity.velocity.y += self.entity.FINAL_GRAVITY * dt
        if not self.entity.grounded: return States.FALL
...
class RunState(State):
    ...
    def process_x_movement(self, dt):
        ...
        if not self.entity.direction: return States.IDLE
...
def calculate_x_velocity(entity, dt):
    ...
    if direction > 0: return min(u + v, target_speed)
    if direction < 0: return max(u + v*entity.x_move_input, target_speed)
    else:
        if sign(u) == -1: return min(u - (v*-1), target_speed*entity.dir)
        else: return max(u - v, target_speed*direction)
